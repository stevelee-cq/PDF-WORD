...
# ========== 熟词库加载（支持任意格式，每行首单词，后为释义）==========
def load_known_words_with_translation(path):
    known_dict = {}
    with open(path, encoding="utf-8") as f:
        for line in f:
            if not line.strip():
                continue
            # 按首个空白（空格或tab）分隔，后面全部为释义
            parts = line.strip().split(None, 1)
            word = parts[0].lower()
            translation = parts[1].strip() if len(parts) > 1 else ""
            known_dict[word] = translation
    return known_dict

KNOWN_WORDS_DICT = load_known_words_with_translation("46merged.txt")
KNOWN_WORDS = set(KNOWN_WORDS_DICT.keys())
...
class PDFWordExtractor(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("英文PDF单词词频提取器（熟词过滤+释义+生词谷歌翻译 | 可视化 | 夜间风格）")
    ...
    def set_dark_theme_style(self):
        self.setStyleSheet("""
            ...
            QTextEdit { background-color: #23272e; border: 1px solid #444; color: #f8f8f2; font-family: Consolas; font-size: 11pt; }
    ...
    def show_known_unknown_words(self):
        ...
        for word, freq in self.word_counter.most_common():
            if word in KNOWN_WORDS:
                trans = KNOWN_WORDS_DICT.get(word, '')
                # 配色绿色，黑体，中文释义高亮
                line = f"<span style='color:#36d900;font-weight:bold;'>{word}</span>&emsp;<span style='color:#ffffff;'>{freq}</span>&emsp;<span style='color:#7ed957;'>{trans}</span>"
                known_lines.append(line)
            else:
                line = f"<span style='color:#FFA726;font-weight:bold;'>{word}</span>&emsp;<span style='color:#ffffff;'>{freq}</span>"
                unknown_lines.append(line)
        # 组装HTML
        content = (f"<div style='font-size:13pt;'>"
                   f"<b><span style='color:#36d900;'>熟词 ({len(known_lines)})</span></b><br>"
                   + "<br>".join(known_lines if known_lines else ["<span style='color:gray;'>无熟词</span>"])
                   + "<hr><b><span style='color:#FFA726;'>生词"
                   f" ({len(unknown_lines)})</span></b><br>"
                   + "<br>".join(unknown_lines if unknown_lines else ["<span style='color:gray;'>无生词</span>"])
                   + "</div>")
        self.text_edit.clear()
